...
const getSalesReport = async (shopId, dateRange) => {
    ...
    const [summaryAgg, byMethod, byStatus, dailyTrend, topProducts] = await Promise.all([
        ...
        Sale_1.Sale.aggregate([
            { $match: { ...matchStage, paymentStatus: { $ne: index_2.PaymentStatus.REFUNDED } } },
...
const getPurchaseReport = async (shopId, dateRange) => {
    const oid = new mongoose_1.Types.ObjectId(shopId);
    const matchStage = {
        shopId: oid,
        isDeleted: false,
...
const getInventoryReport = async (shopId) => {
    const oid = new mongoose_1.Types.ObjectId(shopId);
        ...
        Product_1.Product.aggregate([
            { $match: { shopId: oid, isDeleted: false } },
            {
                $group: {
                    ...
                    totalProducts: { $sum: 1 },
                    activeProducts: {
                        $sum: { $cond: [{ $eq: ["$status", index_2.ProductStatus.ACTIVE] }, 1, 0] },
                    },
                    lowStockProducts: {
                        $sum: { $cond: [{ $eq: ["$status", index_2.ProductStatus.LOW_STOCK] }, 1, 0] },
                    },
                    outOfStockProducts: {
                        $sum: { $cond: [{ $eq: ["$status", index_2.ProductStatus.OUT_OF_STOCK] }, 1, 0] },
                    },
                    discontinuedProducts: {
                        $sum: { $cond: [{ $eq: ["$status", index_2.ProductStatus.DISCONTINUED] }, 1, 0] },
                    },
...
const getExpenseReport = async (shopId, dateRange) => {
    const oid = new mongoose_1.Types.ObjectId(shopId);
    const matchStage = {
        shopId: oid,
        isDeleted: false,
...
const getProfitReport = async (shopId, dateRange) => {
    const oid = new mongoose_1.Types.ObjectId(shopId);
    const saleMatch = {
        shopId: oid,
        isDeleted: false,
...
const getRevenueReport = async (shopId, dateRange) => {
    const oid = new mongoose_1.Types.ObjectId(shopId);
    const matchStage = {
        shopId: oid,
        isDeleted: false,
...
const generatePurchaseCsv = (data) => {
    const rows = [];
...
const buildPdfTable = (doc, headers, rows, startY) => {
...
const generatePdf = (title, headers, rows, summaryRows) => {
...
const buildExcel = async (title, sheets) => {
...
const getReportData = async (shopId, reportType, dateRange) => {
    switch (reportType) {
        case "sales":
        ...
        default:
            throw new AppError_1.AppError(`Invalid report type: ${reportType}`, index_1.HTTP_STATUS.BAD_REQUEST);
    }
};
const generateReport = async (shopId, reportType, format, dateRangeType, startDate, endDate) => {
    ...
    if (format === "csv") {
        ...
        switch (reportType) {
            case "sales":
                ...
                break;
            case "purchases":
                csv = generatePurchaseCsv(data);
                break;
            case "inventory":
                csv = generateInventoryCsv(data);
                break;
            case "expenses":
                csv = generateExpenseCsv(data);
                break;
            case "profit":
                csv = generateProfitCsv(data);
                break;
            case "revenue":
    ...
    if (format === "excel") {
        const sheets = getExcelSheets(reportType, data);
        const excelBuffer = await buildExcel(`${reportType.charAt(0).toUpperCase() + reportType.slice(1)} Report`, sheets);
        return {
            data: excelBuffer,
    ...
    const pdfResult = getPdfData(reportType, data);
    const pdfBuffer = generatePdf(pdfResult.title, pdfResult.headers, pdfResult.rows, pdfResult.summaryRows);
    return {
        data: pdfBuffer,
...
const getPdfData = (reportType, data) => {
    switch (reportType) {
        case "sales": {
            const d = data;
            return {
...
const getExcelSheets = (reportType, data) => {
    switch (reportType) {
        case "sales": {
            const d = data;
            return [
